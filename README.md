...
`dsh-seelog` 是 DeepSeek Harness 的外部 Web 插件。它把一次已下载的 Session Log 转换为可浏览的会话执行图：主 Agent 是一条 3D 主线，子 Agent 从实际启动位置分叉并在结束处汇回；节点、时间和原始日志仍可逐一追溯。
...
- 将长会话保存为浏览器本地的冻结快照。再次下载只新增一个版本，不会修改既有图。
...
2. 点击 `下载并生成快照`。
3. 等待统计信息和图形出现。
4. 右侧 `已下载的可视化图` 会保存这次版本。选择任意版本即可离线重看该图。

新的下载会形成新版本。右侧版本项会显示语义事件数量和相对前一版本的增量；旧版本不会随着实时会话变化。
...
- 概览快照只包含轻量语义字段，不包含模型正文、提示词或工具参数。
- 只有在点击某个节点后，浏览器才请求该事件及其前后两条原始日志。
- 快照存储在当前浏览器的 Local Storage 中，每个根会话最多保留 24 个版本。
- `cordis.patch.yml` 默认最多读取 128 个谱系会话、每个会话最多 20,000 条日志事件。
- 为保障大日志会话的浏览性能，每条轨道最多绘制 400 个交互节点，同时保留输入、模型回复、失败、端点和代表性工具事件。

DSH 的原生 `Session log` 下载按钮目前没有公开的下载完成扩展点，因此插件提供自己的 `下载并生成快照` 操作。该操作是只读的，不会更改会话日志。
...
- 主图无法继续横移：当前快照的可见宽度已经覆盖整个会话；用顶部概览尺可以确认当前范围。
